...
#############################################################
# 预处理yapi接口
./script-test-converage-analyse/handle-yapi-interface.sh ../out/tmp/yapi-interfacelist
# 计算整体覆盖率(以全部yapi接口为参照计算全部jmeter接口的覆盖率)
./script-test-converage-analyse/jmeter-coverage-analyse-all.sh
# 计算项目覆盖率(以全部yapi接口为参照计算单个jmeter项目接口的覆盖率)
./script-test-converage-analyse/jmeter-coverage-analyse-project.sh
# 计算项目覆盖率(以全部jmeter接口为参照计算单个yapi项目接口的覆盖率)
./script-test-converage-analyse/yapi-coverage-analyse-project.sh
#############################################################
